...
class GameBoard:
    ...
    def curretn_player(self, bd = None):
        count_1 = sum(cc == 1 for row in bd for cc in row)
        count_2 = sum(cc == 2 for row in bd for cc in row)
...
class MCTSNode:
    def __init__(self, bd: GameBoard, parent: None, action: None):
        ...
        self.possible_moves = bd.getmoves()  
        self.player = bd.curretn_player(bd.entries)# moves that can be played from this node
        self.visits = 0
        self.wins = 0.0         
